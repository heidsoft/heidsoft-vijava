...
public class InventoryNavigator {
	...
	/**
	 * Retrieve container contents from specified parent recursively if
	 * requested.
	 * 
	 * @param recurse
	 ...
	 */
	public ManagedEntity[] searchManagedEntities(boolean recurse)
	...
	/**
	 * Get the first ManagedObjectReference from current node for the specified
	 * type
	 */
	 ...
	 * @throws InvalidProperty
	 */
	public ManagedEntity[] searchManagedEntities(String[][] typeinfo,
			boolean recurse) throws InvalidProperty, RuntimeFault,
			RemoteException {
		ObjectContent[] ocs = retrieveObjectContents(typeinfo, recurse);
		return createManagedEntities(ocs);
	}

	private ObjectContent[] retrieveObjectContents(String[][] typeinfo,
			boolean recurse) throws InvalidProperty, RuntimeFault,
			RemoteException {
		...
		// 属性收集器
		PropertyCollector pc = rootEntity.getServerConnection()
				.getServiceInstance().getPropertyCollector();

		if (recurse && selectionSpecs == null) {
			AboutInfo ai = rootEntity.getServerConnection()
					.getServiceInstance().getAboutInfo();

			/*
			 * The apiVersion values in all the shipped products "2.0.0" VI 3.0
			 * "2.5.0" VI 3.5 (and u1) "2.5u2" VI 3.5u2 (and u3, u4) "4.0"
			 * vSphere 4.0 (and u1) "4.1" vSphere 4.1 "5.0" vSphere 5.0
			 * ****************************************************
			 */
	...
	private ManagedEntity[] createManagedEntities(ObjectContent[] ocs) {
		...
		for (int i = 0; i < mes.length; i++) {
			...
			mes[i] = MorUtil.createExactManagedEntity(
	...
	/**
	 * Get the ManagedObjectReference for an item under the specified parent
	 * node that has the type and name specified.
	 * 
	 * @param type
		...
		for (int i = 0; i < ocs.length; i++) {
			...
			if (propSet.length > 0) {
				...
				if (name.equalsIgnoreCase(nameInPropSet)) {
					ManagedObjectReference mor = ocs[i].getObj();
					return MorUtil.createExactManagedEntity(
							rootEntity.getServerConnection(), mor);
				}
			}
		}
